...
def load_protein_to_split_json(path: str) -> Dict[str, str]:
    """
    Load per-protein train/val/test labels from a merged Phase-0 bundle.

    Expected JSON shape: top-level ``protein_to_split`` mapping
    protein_id -> \"train\" | \"val\" | \"test\" (e.g. UniRef-aware split + graphless removal).
    """
